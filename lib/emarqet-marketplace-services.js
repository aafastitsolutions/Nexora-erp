const SERVICE_CATALOG = [
  ...
  },
  {
    ...
    name: "Raport istoric auto",
    vertical_code: "auto",
    category: "raport",
    provider_key: "carvertical_autodna_cebia",
    fulfillment_type: "partner_api_or_affiliate",
    pricing_model: "paid_or_affiliate",
    price_amount: null,
    included_in_plan: "",
    requires_partner: 1,
    cta_label: "Verifică istoricul",
    badge_label: "Istoric verificat",
    description: "Raport istoric pentru daune, kilometri, furt, titluri, service sau alte date disponibile la furnizor.",
    sort_order: 30
  },
  {
...
export function createMarketplaceServiceRequest(db, companyId, data = {}) {
  ...
  const result = db.prepare(`
    INSERT INTO emarqet_service_requests
      (company_id, service_id, listing_id, vertical_id, service_code, provider_key, requester_name, requester_email, requester_phone, status, amount, currency, request_payload_json, result_json, notes, created_by)
    VALUES
      (@company_id, @service_id, @listing_id, @vertical_id, @service_code, @provider_key, @requester_name, @requester_email, @requester_phone, @status, @amount, @currency, @request_payload_json, @result_json, @notes, @created_by)
  `).run({
