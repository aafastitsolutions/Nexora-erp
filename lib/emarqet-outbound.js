...
const TECHNICAL_EMAIL_DOMAIN_SUFFIXES = [
  "amazonses.com",
  "hubspotemail.net",
  "ingest.de.sentry.io",
  "intercom-mail.com",
...
const TECHNICAL_EMAIL_LOCAL_PARTS = new Set([
  "abuse",
  "bounce",
  "bounces",
  "daemon",
...
export function ensureEmarqetOutboundSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS emarqet_outbound_email_events(
      ...
      cc_email TEXT,
      mailbox TEXT,
      message_type TEXT DEFAULT 'partner_outreach',
  ...
  `);
  ensureColumn(db, "emarqet_outbound_email_events", "from_email", "TEXT");
  ensureColumn(db, "emarqet_outbound_email_events", "reply_to_email", "TEXT");
  ensureColumn(db, "emarqet_outbound_email_events", "cc_email", "TEXT");
  ensureColumn(db, "emarqet_outbound_email_events", "mailbox", "TEXT");
  ensureColumn(db, "emarqet_outbound_email_events", "message_type", "TEXT DEFAULT 'partner_outreach'");
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_emarqet_outbound_events_mailbox
      ON emarqet_outbound_email_events(company_id, mailbox, status, created_at DESC);
  `);
}
...
export function isGenericBusinessEmail(email = "") {
  ...
  if (TECHNICAL_EMAIL_DOMAIN_SUFFIXES.some((suffix) => domain === suffix || domain.endsWith(`.${suffix}`))) return false;
  if (TECHNICAL_EMAIL_LOCAL_PARTS.has(local)) return false;
  const role = local.split(/[.+_-]/)[0];
  if (TECHNICAL_EMAIL_LOCAL_PARTS.has(role)) return false;
...
function segmentPitch(templateKey = "") {
  ...
  if (key === "real_estate") {
    return [
      "Pentru agentii imobiliare putem activa o pagina publica de partener, ca un mini-site al agentiei, cu toate anunturile grupate frumos sub acelasi profil.",
      "Putem importa anunturile initiale din site, CSV, XML sau feed, cu filtre dedicate pentru apartamente, case, terenuri si spatii comerciale.",
      "In perioada de lansare oferim configurarea si importul initial gratuit, iar cererile venite din anunturi pot fi urmarite centralizat in Nexora."
    ];
  }
  if (key === "tourism") {
    return [
...
export function buildEmarqetOutboundMessage(lead = {}, options = {}) {
  ...
  const officeEmail = safeText(options.officeEmail || emarqetOfficeEmail());
  const supportEmail = safeText(options.supportEmail || emarqetSupportEmail());
  ...
  const subjectMap = {
    auto_dealer: "Propunere colaborare E-MARQET pentru stoc auto",
  ...
  const autoIntro = city
    ? `Va scriem din partea e-Marqet pentru ca vrem sa construim o colaborare cu parcuri auto si dealeri de autorulate, iar ${companyName} din ${city} pare potrivit pentru prima etapa.`
    : `Va scriem din partea e-Marqet pentru ca vrem sa construim o colaborare cu parcuri auto si dealeri de autorulate, iar ${companyName} pare potrivit pentru prima etapa.`;
  const pitch = templateKey === "auto_dealer"
    ? [
    ...
    : segmentPitch(templateKey);
  const introLine = templateKey === "auto_dealer" ? autoIntro : intro;
  const actionLine = templateKey === "auto_dealer"
  ...
  const brandLine = templateKey === "auto_dealer"
    ? "Suntem echipa e-Marqet, o platforma romaneasca de anunturi si parteneri business conectata la Nexora."
  ...
  const text = [
    ...
    actionLine,
    "",
    `Pentru parteneriate ne puteti contacta la ${officeEmail}. Pentru suport, importuri sau tichete tehnice: ${supportEmail}.`,
    "",
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.55;color:#0f172a;max-width:680px">
      ...
      <p>Pentru parteneriate ne puteti contacta la <a href="mailto:${escapeHtml(officeEmail)}">${escapeHtml(officeEmail)}</a>. Pentru suport, importuri sau tichete tehnice: <a href="mailto:${escapeHtml(supportEmail)}">${escapeHtml(supportEmail)}</a>.</p>
      ...
      <p>Cu respect,<br>Echipa e-Marqet<br><a href="mailto:${escapeHtml(officeEmail)}">${escapeHtml(officeEmail)}</a><br><a href="mailto:${escapeHtml(supportEmail)}">${escapeHtml(supportEmail)}</a><br><a href="${escapeHtml(`${publicBaseUrl}/parteneri`)}">${escapeHtml(`${publicBaseUrl}/parteneri`)}</a></p>
    </div>
  `;
  return {
