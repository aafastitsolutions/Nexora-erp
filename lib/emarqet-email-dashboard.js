...
export function loadEmarqetEmailDashboard(db, companyId, options = {}) {
  ...
  const inbound = [
    ...ticketRows.map((row) => ({
      ...
      subject: safeText(row.service_name || row.service_code || "Tichet suport"),
      contactName: safeText(row.requester_name),
      phone: safeText(row.requester_phone),
      status: safeText(row.status),
      details: [row.listing_title, row.listing_code, row.service_category].map(safeText).filter(Boolean).join(" · "),
