...
export function maybeGenerateBillingInvoice(db, paymentId, options = {}) {
  ...
  const allowConsumerWithoutFiscalId = Boolean(paymentMetadata.allow_consumer_without_fiscal_id);
  ...
  if (isExternalPayer && !allowConsumerWithoutFiscalId && isRomanianCountry(payerCountry) && !hasUsableRomanianFiscalId(payerCui)) {
    db.prepare(`
      UPDATE billing_payments
      SET invoice_generation_status='blocked_missing_fiscal_id',
          failure_reason=CASE
            WHEN failure_reason IS NULL OR failure_reason=''
              THEN 'Lipseste CUI/CNP valid pentru factura automata.'
            ELSE failure_reason || ' | Lipseste CUI/CNP valid pentru factura automata.'
          END,
          updated_at=datetime('now')
      WHERE id=?
    `).run(normalizedPaymentId);
