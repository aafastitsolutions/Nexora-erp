import { ERP_MODULES } from "../src/config/erp-modules.js";
...
export const LEGACY_MODULE_GROUPS = {
  crm: [
    "dashboard",
    "clients",
    "quotes",
...
export const LEGACY_MODULE_KEYS = Object.values(LEGACY_MODULE_GROUPS).flat();

export const LEGACY_MODULE_DEFINITIONS = [
  ...
  { key: "projects", label: "Proiecte", group: "erp" },
...
export const CORE_ALWAYS_INCLUDED_MODULES = ["dashboard", "settings"];
export const DEMO_PLAN_CODE = "demo";

const ERP_MODULE_BY_KEY = new Map(ERP_MODULES.map((module) => [module.key, module]));

export const MODULE_DEFINITIONS = Object.entries(MODULE_GROUPS).flatMap(([group, moduleKeys]) =>
  moduleKeys.map((moduleKey) => {
    const module = ERP_MODULE_BY_KEY.get(moduleKey);
    return {
      key: moduleKey,
      label: module?.label || moduleKey,
      group,
      path: module?.path || "",
      legacyModuleKeys: Array.isArray(module?.legacyModuleKeys) ? module.legacyModuleKeys : []
    };
  })
);

const CURRENT_MODULE_SET = new Set(ALL_MODULE_KEYS);
const LEGACY_MODULE_SET = new Set(LEGACY_MODULE_KEYS);
const PRIMARY_LEGACY_TO_CURRENT_MODULE = {
  dashboard: "dashboard",
  clients: "crm",
  quotes: "sales",
  contracts: "sales",
...
export function toCurrentModuleKeys(values = []) {
  const result = [];
  for (const rawValue of values || []) {
    const moduleKey = String(rawValue || "").trim();
    if (!moduleKey) continue;
    if (CURRENT_MODULE_SET.has(moduleKey)) {
      result.push(moduleKey);
      continue;
    }
    const mappedKey = PRIMARY_LEGACY_TO_CURRENT_MODULE[moduleKey];
    if (mappedKey) result.push(mappedKey);
  }
  return uniqueModuleKeys(result);
}

export function expandModuleKeysForAccess(values = []) {
  const expanded = [];
  for (const rawValue of values || []) {
    const moduleKey = String(rawValue || "").trim();
    if (!moduleKey) continue;
    expanded.push(moduleKey);

    if (CURRENT_MODULE_SET.has(moduleKey)) {
      const module = ERP_MODULE_BY_KEY.get(moduleKey);
      expanded.push(...(module?.legacyModuleKeys || []));
      continue;
    }

    if (LEGACY_MODULE_SET.has(moduleKey)) {
      const mappedKey = PRIMARY_LEGACY_TO_CURRENT_MODULE[moduleKey];
      if (mappedKey) expanded.push(mappedKey);
    }
  }
  return uniqueModuleKeys(expanded);
}

export function hasModuleKeyAccess(values = [], requiredModuleKey = "") {
  const required = String(requiredModuleKey || "").trim();
  if (!required) return true;
  return expandModuleKeysForAccess(values).includes(required);
}
...
export const DEFAULT_PLAN_DEFINITIONS = [
  {
    ...
    module_keys: ALL_MODULE_KEYS.filter((key) => key !== "settings"),
...
function orderedModuleUniverse(allowedModules = ALL_MODULE_KEYS) {
  const allowedSet = new Set(toCurrentModuleKeys(allowedModules || []).map(String));
...
export function orderModules(moduleKeys = [], allowedModules = ALL_MODULE_KEYS) {
  const allowedSet = new Set(toCurrentModuleKeys(allowedModules || []).map(String));
  const requestedSet = new Set(toCurrentModuleKeys(moduleKeys).filter((key) => allowedSet.has(key)));
...
function baseAccessibleModules(allowedModules = ALL_MODULE_KEYS, { companyIsDemo = false } = {}) {
  ...
  return CORE_ALWAYS_INCLUDED_MODULES.filter((moduleKey) => {
    if (companyIsDemo && moduleKey === "settings") return false;
...
export function normalizeCompanyModules(requestedModules, allowedModules = ALL_MODULE_KEYS, plan = {}, { companyIsDemo = false } = {}) {
  const orderedAllowed = orderedModuleUniverse(toCurrentModuleKeys(allowedModules));
  const requested = Array.isArray(requestedModules) && requestedModules.length
    ? orderModules(requestedModules, orderedAllowed)
    : orderedAllowed;
  const baseModules = baseAccessibleModules(orderedAllowed, { companyIsDemo });
  const baseSet = new Set(baseModules);
  const remainingModules = requested.filter((moduleKey) => !baseSet.has(moduleKey) && (!companyIsDemo || moduleKey !== "settings"));
...
export function normalizeUserModules(requestedModules, activeCompanyModules = ALL_MODULE_KEYS, plan = {}, roleModules = ALL_MODULE_KEYS, { companyIsDemo = false } = {}) {
  const activeCurrentModules = toCurrentModuleKeys(activeCompanyModules);
  const roleAllowedModules = orderModules(roleModules, activeCurrentModules);
  const requested = Array.isArray(requestedModules) && requestedModules.length
