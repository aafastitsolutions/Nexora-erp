...
const PROFILE_FIELDS = [
  ...
  },
  {
    ...
    tokens: ["legal_representative", "company_representative", "company.representative", "reprezentant_legal", "reprezentant_legal_nume"],
    pdfAliases: ["legal_representative", "legal representative", "company representative", "reprezentant legal", "reprezentant firma", "reprezentant societate", "administrator societate", "administrator firma"]
  },
  {
...
const DOCX_CONTEXTUAL_FIELDS = [
  {
    ...
    value(profile) {
      return [
        ...
        profileValue(profile, "company_cui") ? `CUI ${profileValue(profile, "company_cui")}` : "",
        profileValue(profile, "company_address")
      ].filter(Boolean).join(", ");
    },
    patterns: [
      `Beneficiar\\s*:\\s*(${DOCX_BLANK_PATTERN})\\s*(?=\\(\\s*denumirea\\s*,?\\s*codul\\s+de\\s+(?:i|î)nregistrare\\s+fiscal(?:a|ă))`
    ]
  },
  {
    key: "legal_representative",
    label: "Reprezentant legal",
    patterns: [
      `Subsemnatul\\s*\\(a\\)\\s*,\\s*(${DOCX_BLANK_PATTERN})`,
  ...
  },
  {
    ...
    label: "Emitent CI reprezentant",
    patterns: [
      `eliberat\\s*\\((?:a|ă)\\)\\s+de\\s*(${DOCX_BLANK_PATTERN})`
    ]
  },
  {
    key: "company_name",
