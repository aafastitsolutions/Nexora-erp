...
function messageDrafts(candidate = {}, lead = {}, insight = {}) {
  ...
  const city = safeText(candidate.city || lead.city);
  const county = safeText(candidate.county || lead.county);
  const locationText = [city, county].filter(Boolean).join(", ") || "Romania";
  const areaText = county ? `din ${county}` : "din Romania";
  ...
  const shortEmail = [
    "Buna ziua,",
    "",
    `Va scriem din partea E-MARQET pentru ca vrem sa construim o colaborare cu parcuri auto si dealeri de autorulate ${areaText}, iar ${name} din ${locationText} pare potrivit pentru prima etapa.${sourceLine ? ` ${sourceLine}` : ""}`,
    "",
...
function isLikelyAutoSalesLead(row = {}) {
  ...
  const officialRepresentative = [
    ...
    "volvo",
    "porsche"
  ].some((token) => key.includes(normalizeKey(token)));
  if (officialRepresentative && !["0", "false", "no", "nu"].includes(String(process.env.EMARQET_AUTO_EXCLUDE_OFFICIAL_REPS ?? "true").trim().toLowerCase())) {
    return false;
  }
  const excluded = [
    "dezmembr",
...
export function listCampaignApprovalRows(db, companyId = 0, campaignId = 0, limit = 50) {
  ...
  return db.prepare(`
    ...
    WHERE l.company_id=?
      ...
      AND (COALESCE(cp.county,'')='' OR c.county=cp.county)
