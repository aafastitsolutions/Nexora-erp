...
const ROMANIA_OWNER_PLANS = "Basic 99 lei/lună, Premium 149 lei/lună, Business 249 lei/lună";
const INTERNATIONAL_OWNER_PLANS = "Basic 19 EUR/month, Premium 29 EUR/month, Business 59 EUR/month";
const INTERNATIONAL_LISTING_PLANS = [
  ["Basic", "19 EUR / month"],
  ["Premium", "29 EUR / month"],
  ["Business", "59 EUR / month"]
];
...
function loadEmailBlocks(companyId) {
  ...
  for (const row of rows) {
    ...
    if (row.direction === "outbound" || row.direction === "bounce") {
      if (leadId) sentLeadIds.add(leadId);
      if (email) sentEmails.add(email);
      continue;
    }

    const failedAt = sqliteDateMs(row.received_at);
    const recentFailure = failedAt ? now - failedAt < recentWindowMs : true;
  ...
  return { failedEmails, failedLeadIds, sentEmails, sentLeadIds };
}

function loadLeads(companyId, { limit, sources, minScore, country, international = false, includeExcludedBrands = false }) {
  ...
  const candidates = db.prepare(`
    SELECT id, company_id, name, city, county, country, property_type, email, score,
  ...
  const blocks = loadEmailBlocks(companyId);
  ...
  for (const lead of candidates) {
    ...
    if (
      ...
      || blocks.sentLeadIds.has(Number(lead.id))
      || blocks.sentEmails.has(email)
...
function markSent(companyId, lead, info, subject, cc, replyTo, textBody = "") {
  db.transaction(() => {
    db.prepare(`
      UPDATE travel_leads
    ...
    db.prepare(`
      ...
      VALUES (?, ?, 'status_changed', 'Status schimbat', 'nou -> contactat', datetime('now'))
    ...
    `).run(
      ...
      safeText(lead.email),
      subject,
      safeText(textBody) || "Email outreach Trevoro trimis din platformă."
    );
  })();
}
...
async function main() {
  ...
  const leads = loadLeads(companyId, { limit, sources, minScore, country, international: internationalRun, includeExcludedBrands });
  ...
  for (let index = 0; index < leads.length; index += 1) {
    ...
    try {
      ...
      markSent(companyId, lead, info, message.subject, cc, replyTo, message.text);
  ...
  fs.writeFileSync(reportPath, JSON.stringify({ sentAt: new Date().toISOString(), limit, source, sources, country: country || "all", international: internationalRun, minScore, dryRun, allowPartial, stopOnThrottle, includeExcludedBrands, stoppedEarly, pauseRequested, claimUrl: reportClaimUrl, cc: maskEmail(cc), replyTo: maskEmail(replyTo), report }, null, 2));
